...
Token 需要配置为系统环境变量 `WOLAI_MCP_TOKEN`，配置后重启 WorkBuddy 即可生效。以下提供三种方法，推荐**方法 A**，最直观。
...
if (-not $env:WOLAI_MCP_TOKEN) {
    ...
    Write-Host ""
    Write-Host "自行配置环境变量（Token 不经过对话）："
    Write-Host "  - Win+R 输入 sysdm.cpl → 高级 → 环境变量 → 新建 WOLAI_MCP_TOKEN"
    Write-Host "  - 或在 PowerShell 中自行执行：[System.Environment]::SetEnvironmentVariable('WOLAI_MCP_TOKEN','sk-xxx','User')"
    Write-Host ""
